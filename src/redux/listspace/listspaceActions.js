            ...
            .then(response => {
                ...
                if (+serverResponse.status) {
                    ...
                    let stepFive = {
                        ...
                        your_earnings: +serverResponse.store_earnings_deposit,
                        fees: +serverResponse.details.store_fees,
                        token: JSON.parse(localStorage.getItem("stashGuruToken")),
...
const listingCountRequest = () => {
    return {
        type: LISTING_COUNT_REQUEST
    }
}

const listingCountSuccess = (response) => {
    return {
        type: LISTING_COUNT_SUCCESS,
        payload: response
    }
}

const listingCountFailure = (response) => {
    return {
        type: LISTING_COUNT_FAILURE,
        payload: response
    }
}
...
export const getlistingCount = (reqData = {}, query = "") => {
    ...
    return async (dispatch) => {
        dispatch(listingCountRequest());

        reqData.token = await validateClientToken();


        await axios.post(`${config.apiUrl}/front/users/listing_count?${query}`, reqData, requestConfig)
            .then(response => {
                const serverResponse = response.data;
                if (+serverResponse.status) {

                    dispatch(listingCountSuccess(serverResponse.data.count));

                }
                else {
                    dispatch(listingCountFailure(serverResponse.message));
                }

            }).catch(error => {
                const errorMsg = error.message;
                dispatch(listingCountFailure(errorMsg));
